...
public abstract class AbActivity extends FragmentActivity {
	...
	private View freeDialogView = null;
	/** 底部弹出的Dialog的View. */
	...
	public static final int TOASTNORMAL = 0;// 一般toast
	public static final int TOASTSUCCESS = -1;// 成功的toast
	public static final int TOASTHINT = -2;// 提示
	public static final int TOASTERROR = -3;// 失败
	public static final int TOASTCENTER = -4;// 失败
	...
	private Handler baseHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case AbConstant.SHOW_TOAST:
				MyToast.customToast(AbActivity.this, Toast.LENGTH_SHORT, msg
	 ...
	 *            文本
	 */
	public void showToast(int type, String text) {
		switch (type) {
		case TOASTNORMAL:
			...
			break;
		case TOASTCENTER:
			MyToast.centerToast(this, "" + text);
			break;
		case TOASTSUCCESS:
			MyToast.SuccessToast(this, text);
			break;
		case TOASTHINT:
			MyToast.WarningToast(this, text);
			break;
		case TOASTERROR:
			MyToast.ErrorToast(this, text);
			break;
	...
	public void showToast(String text) {
		MyToast.customToast(this, Toast.LENGTH_SHORT, text);
	}
	...
	public void showToast(int resId) {
		MyToast.customToast(this, Toast.LENGTH_SHORT, ""
				+ this.getResources().getText(resId));
	}
	...
	public void showProgressDialog(String message) {
		...
		if (progressView == null) {
			...
			// gif = (GifView)progressView.findViewById(R.id.gif);
			// loadImg = (ImageView)progressView.findViewById(R.id.load);
			// AnimationDrawable animationDrawable = (AnimationDrawable)
			// loadImg.getBackground();
		...
		mProgressDialog.setContentView(progressView, layoutParamsFF);
	}

	/**
	 * 设置弹框的高 *********调用该方法后弹框背景全透明
	 * 
	 * @param y
	 */
	public void setTopDailogY(int my) {
		if (mTopDialog == null) {
			mTopDialog = new Dialog(this, R.style.dialog);
		}
		mTopDialog.getWindow().getAttributes().y = my;
	}
	...
	public void showDialog(int id, View view) {
		...
		if (id == AbConstant.DIALOGBOTTOM) {
			...
			if (mBottomDialog == null) {
				mBottomDialog = new Dialog(this, R.style.dialog);
				setDialogLayoutParams(mBottomDialog, 0, Gravity.BOTTOM
						| Gravity.CENTER_HORIZONTAL);
			}
			mBottomDialog.setContentView(mBottomDialogView, new LayoutParams(
					diaplayWidth, LayoutParams.WRAP_CONTENT));
			showDialog(id);
			...
			mTopDialog.setContentView(mTopDialogView, new LayoutParams(
					diaplayWidth - dialogPadding, LayoutParams.WRAP_CONTENT));
			showDialog(id);
		} else if (id == AbConstant.DIALOGTOP_LEFT) {
			mTopDialogView = view;
			if (mTopDialog == null) {
				mTopDialog = new Dialog(this, R.style.dialog);
				setDialogLayoutParams(mTopDialog, dialogPadding, Gravity.TOP
			...
			mTopDialog.getWindow().setGravity(Gravity.TOP | Gravity.LEFT);
			mTopDialog.setContentView(mTopDialogView, new LayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
			showDialog(AbConstant.DIALOGTOP);
		} else if (id == AbConstant.DIALOGTOP_CENTER) {
			mTopDialogView = view;
			if (mTopDialog == null) {
				mTopDialog = new Dialog(this, R.style.dialog);
				setDialogLayoutParams(mTopDialog, dialogPadding, Gravity.TOP
			...
			mTopDialog.getWindow().setGravity(
					Gravity.TOP | Gravity.CENTER_HORIZONTAL);
			mTopDialog.setContentView(mTopDialogView, new LayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
			showDialog(AbConstant.DIALOGTOP);
		} else if (id == AbConstant.DIALOGTOP_RIGHT) {
			mTopDialogView = view;
			if (mTopDialog == null) {
				mTopDialog = new Dialog(this, R.style.dialog);
				setDialogLayoutParams(mTopDialog, dialogPadding, Gravity.TOP
			...
			mTopDialog.getWindow().setGravity(Gravity.TOP | Gravity.RIGHT);
			mTopDialog.setContentView(mTopDialogView, new LayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
			showDialog(AbConstant.DIALOGTOP);
		} else {
	...
	public void showDialog(String title, String msg,
			DialogInterface.OnClickListener mOkOnClickListener,final DialogInterface.OnClickListener mCancleOnClickListener) {
		...
		builder.setNegativeButton("取消", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if(mCancleOnClickListener!=null){
					mCancleOnClickListener.onClick(dialog, which);
				}
			}
		});
		builder.create().show();
	...
	public void showDialog(String title, String msg,
		...
		showDialog(title, msg, mOkOnClickListener, null);
	}
	...
	public void ShowFreeDialog(View view, float x, float y) {
		if (freeDialog == null) {
			freeDialog = new Dialog(this);
			freeDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
			Window window = freeDialog.getWindow();
